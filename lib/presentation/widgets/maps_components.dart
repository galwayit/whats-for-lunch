...
// Cluster manager temporarily disabled due to compatibility issues
...
/// Enhanced Maps components with clustering, custom markers, directions, and offline support
/// Optimized for restaurant discovery with dietary compatibility indicators
/// Performance target: <3s load time, 60fps navigation, smart marker clustering

/// Place item for restaurant mapping (clustering temporarily disabled)
class RestaurantPlace {
  ...
  }) : _location = location, id = restaurant.placeId;

  LatLng get location => _location;

  String get geohash => id;
}

/// Enhanced maps view with clustering and advanced features
class UXMapsView extends StatefulWidget {
  ...
  @override
  State<UXMapsView> createState() => _UXMapsViewState();
}

class _UXMapsViewState extends State<UXMapsView> with TickerProviderStateMixin {
  ...
  // Clustering (temporarily disabled)
  // cluster_manager.ClusterManager? _clusterManager;
  List<RestaurantPlace> _restaurantPlaces = [];
  ...
  @override
  void didUpdateWidget(UXMapsView oldWidget) {
  ...
  /// Initialize cluster manager for restaurant grouping (temporarily disabled)
  void _initializeClusterManager() {
    // Clustering temporarily disabled due to package compatibility issues
    /*
    if (widget.enableClustering && !_isDisposed) {
      _clusterManager = cluster_manager.ClusterManager<RestaurantPlace>(
        _restaurantPlaces,
        _updateMarkers,
        markerBuilder: _getClusterMarker,
  ...
  /// Create cluster marker for grouped restaurants (temporarily disabled)
  /*
  Future<Marker> _getClusterMarker(cluster_manager.Cluster<RestaurantPlace> cluster) async {
    final count = cluster.count;
    final color = count < 5 ? Colors.green : count < 10 ? Colors.orange : Colors.red;
    
    final markerIcon = await _createClusterMarker(count.toString(), color);
  ...
  /// Create cluster marker bitmap
  Future<BitmapDescriptor> _createClusterMarker(String text, Color color) async {
    final pictureRecorder = PictureRecorder();
    final canvas = Canvas(pictureRecorder);
    const size = 60;
  ...
  /// Zoom to cluster bounds
  void _zoomToCluster(cluster_manager.Cluster<RestaurantPlace> cluster) {
  ...
  */
  ...
  void _initializeMarkers() {
    ...
    try {
      _setupRestaurantPlaces();
      _updateUserLocationMarker();
      _updateRadiusCircle();
      
      // Clustering temporarily disabled due to package compatibility issues
      // if (widget.enableClustering && _clusterManager != null) {
      //   _clusterManager!.setItems(_restaurantPlaces);
      // } else {
  ...
  /// Setup restaurant places for clustering
  void _setupRestaurantPlaces() {
    _restaurantPlaces.clear();
    
    for (final restaurant in widget.restaurants) {
      if (restaurant.latitude != null && restaurant.longitude != null) {
        _restaurantPlaces.add(
          RestaurantPlace(
  ...
  /// Create individual restaurant markers (non-clustered)
  ...
  /// Callback for cluster manager updates
  void _updateMarkers(Set<Marker> markers) {
    if (_isDisposed || !mounted) return;
    
    // Add user location marker to clustered markers
  ...
  /// Legacy marker icon method for fallback
  BitmapDescriptor _getMarkerIcon(SafetyLevel safetyLevel, double dietaryScore) {
    double hue;
    
    if (safetyLevel == SafetyLevel.warning) {
      hue = BitmapDescriptor.hueRed;
    } else if (safetyLevel == SafetyLevel.caution) {
      hue = BitmapDescriptor.hueOrange;
    } else if (dietaryScore >= 0.8) {
  ...
  Future<void> _getDirections(Restaurant restaurant) async {
    ...
    try {
      ...
      // In production, you would use a routing API
  ...
  String _getMarkerSnippet(Restaurant restaurant) {
    final rating = restaurant.rating != null ? '⭐ ${restaurant.rating!.toStringAsFixed(1)}' : '';
    final price = restaurant.priceLevel != null ? ' • ${r'$' * restaurant.priceLevel!}' : '';
    final distance = restaurant.distanceFromUser != null 
        ? ' • ${restaurant.distanceFromUser!.toStringAsFixed(1)}km'
        : '';
    
    return '$rating$price$distance';
  }
  ...
  void _onMapCreated(GoogleMapController controller) async {
    ...
    // Set up cluster manager with map controller (clustering temporarily disabled)
    // if (widget.enableClustering && _clusterManager != null) {
  ...
  /// Handle camera movement for clustering (temporarily disabled)
  ...
  /// Handle camera idle for clustering (temporarily disabled)
  void _onCameraIdle() {
    // if (widget.enableClustering && _clusterManager != null) {
  ...
  void dispose() {
    ...
    if (_controller != null) {
      try {
        ...
        LoggingService().warning('Error disposing Google Maps controller', tag: 'Maps', error: e);
      }
    }
...
/// Map controls for radius adjustment and view switching
class UXMapControls extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Radius control
            Text(
              'Search Radius: ${currentRadius.toStringAsFixed(1)}km',
              style: theme.textTheme.titleSmall?.copyWith(
                fontWeight: FontWeight.w500,
              ),
            ),
            ...
            Row(
              children: [
                ...
                Semantics(
                  ...
                  child: IconButton(
                    onPressed: isLocationLoading ? null : onLocationPressed,
...
/// Reduces vertical space while maintaining functionality
